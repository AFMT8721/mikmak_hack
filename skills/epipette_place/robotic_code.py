...
# places); intentionally distinct from the shared RIGHT_ARM_STOW_JOINTS in utils.
...
def epipette_place(
    ...
    # This pipette is no longer in hand — clear its live_state flag (see the
    # matching write in epipette_grab). Best-effort; never fail the place.
    try:
        from execution.execution_functions import set_world_state

        set_world_state(object_id, {"in_hand": False})
    except Exception as e:
        print_log(f"epipette_place: in_hand tracking failed: {e}")
