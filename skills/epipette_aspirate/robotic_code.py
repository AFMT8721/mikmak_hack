...
# for every other pipette. Negative = descend less deep.
...
def epipette_aspirate(
    ...
    Args:
        ...
        mix_cycles: Trituration cycles to run **while the tip is still at depth**,
            before the retract. 0 (default) skips mixing entirely. Mixing here rather
            than from a separate node is what keeps the tip submerged — this skill
            always retracts to the hover height when it finishes, so a mix chained
            after it would pump air.
    ...
    # Per-pipette trim (see DEPTH_TRIM_BY_PIPETTE). Clamped at 0 so a trim larger
    # than the depth can never lift the dip above the well top.
    trim = DEPTH_TRIM_BY_PIPETTE.get(pipette_name, 0.0)
    if trim:
        trimmed = max(0.0, round(depth + trim, 6))
        print_log(f"epipette_aspirate: pipette '{pipette_name}' depth trim {trim:+.4f} m -> {depth:.5f} becomes {trimmed:.5f} m")
        if trimmed == 0.0:
            print_log(f"epipette_aspirate: WARNING trim {trim:+.4f} m >= depth {depth:.5f} m; clamped to 0 (dipping to the well top)")
        depth = trimmed
    ...
    # the last `slow_zone_m`. The whole 15 cm used to run at the rim speed.
    move_arm_ramped(
        arm="left_arm",
    ...
    _epipette_aspirate_helper(name=epipette_device(pipette), volume=volume, speed=speed)
    time.sleep(0.5)

    # Run log: one timestamped structured entry per aspirate, with full well info
    # (print_log adds the UTC `t`, node_id, and label from the execution context).
    print_log(
        f"aspirate {volume} uL from {object_display_name(object, object_id)} / well {anchor_name} "
        f"(pipette {object_display_name(pipette, pipette_id)}, speed {speed}, cal ({dx},{dy}))",
        runlog=True,
        runlog_type="aspirate",
    )

    # 5) Optional mix, still at depth. This has to happen before step 6 — the tip is
    # submerged right now, and the retract below puts it `approach_height` above the
    # liquid. epipette_mix works the plunger only and never moves the arm.
    ...
    # 6) Retract back to the hover height.
